...
export const metadata: Metadata = {
  ...
  description: 'Discover approved temples and meditation centres listed on Vallalar Jeevakarunyam.',
}

export default async function TemplePage() {
  const supabase = await createClient()
  const { data: listings, error } = await supabase.from('listings').select(`*, states (name), districts (name)`).eq('listing_type', 'temple').eq('status', 'approved').eq('verification_status', 'verified').order('created_at', { ascending: false })
  ...
  return <main className="min-h-screen bg-gray-50">
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-10">
      <div className="mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Temples & Meditation Centres</h1>
        <p className="text-gray-600 mt-3 max-w-2xl leading-7">Discover approved temples and meditation centres supporting spiritual practice, compassion and service to living beings. Please confirm visiting hours directly before travelling.</p>
      </div>
