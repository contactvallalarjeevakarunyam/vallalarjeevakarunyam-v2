...
export default async function MapPage() {
  ...
  const { data: listings, error } = await supabase
    .from('listings')
    .select(`id, name, listing_type, google_maps_url, latitude, longitude, village, taluk, states(name), districts(name)`)
    .eq('status', 'approved')
    .order('name')

  const allListings = listings || []
  const markerListings = allListings
    ...
    .map((listing) => ({
      ...
      taluk: listing.taluk,
      state_name: listing.states?.name || null,
      district_name: listing.districts?.name || null,
    }))
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-3 max-w-3xl leading-7">Explore approved Annadhanam centres, Jeeva Samadhis, temples, affordable stays, medical services and community service groups on one map.</p>
        </div>
