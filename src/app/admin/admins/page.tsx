...
export default async function AdminManagementPage() {
  ...
  async function addAdmin(formData: FormData) {
    ...
    await db.from('admin_invites').upsert({ email, role: 'admin', status: 'pending' }, { onConflict: 'email' }).select()
  ...
  async function revokeInvite(formData: FormData) {
    ...
    const { data: actorAdmin } = await db.from('admins').select('role').eq('user_id', actor.id).maybeSingle()
    if (actorAdmin?.role !== 'super_admin') return
    await db.from('admin_invites').update({ status: 'revoked' }).eq('id', id).eq('status', 'pending')
    revalidatePath('/admin/admins')
  }
  ...
  return <main className="max-w-5xl mx-auto p-6">
    ...
    <div className="mt-6 mb-8"><p className="text-sm font-semibold text-emerald-700">Super Admin</p><h1 className="text-3xl font-bold text-gray-900 mt-1">Admin Management</h1><p className="text-gray-600 mt-2">Control who is authorized to administer Vallalar Jeevakarunyam. Passwords remain private to each administrator.</p></div>
    <form action={addAdmin} className="bg-white border border-gray-200 rounded-xl p-5 mb-8"><h2 className="text-lg font-bold text-gray-900">Add future admin</h2><p className="text-sm text-gray-600 mt-1 mb-4">Enter their email now. Their admin access can be activated after they create their own login account.</p><div className="flex flex-col sm:flex-row gap-3"><input name="email" type="email" required placeholder="admin@example.com" className="flex-1 border border-gray-300 rounded-lg px-4 py-2.5"/><button className="bg-emerald-700 text-white font-semibold rounded-lg px-5 py-2.5 hover:bg-emerald-800">Add Admin Email</button></div></form>
    ...
    <section><h2 className="text-xl font-bold text-gray-900 mb-4">Admin Emails / Invitations</h2><div className="space-y-3">{(invites || []).map((invite) => <div key={invite.id} className="bg-white border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><p className="font-semibold text-gray-900">{invite.email}</p><p className="text-sm text-gray-500 mt-1">Status: {invite.status}</p></div>{invite.status === 'pending' && <form action={revokeInvite}><input type="hidden" name="id" value={invite.id}/><button className="border border-red-300 text-red-700 px-4 py-2 rounded-lg text-sm font-semibold hover:bg-red-50">Revoke</button></form>}</div>)}</div></section>
  </main>
}
