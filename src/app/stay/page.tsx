...
export default async function StayPage() {
  ...
  const { data: listings, error } = await supabase.from('listings').select(`*, states (name), districts (name)`).eq('listing_type', 'stay').eq('status', 'approved').eq('verification_status', 'verified').order('created_at', { ascending: false })
