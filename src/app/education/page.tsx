...
export const metadata: Metadata = {
  ...
  description: 'Find approved free and affordable education providers, schools, colleges, learning centres, coaching and skill-development services listed on Vallalar Jeevakarunyam.',
}

export default async function EducationPage() {
  const supabase = await createClient()
  const { data: listings, error } = await supabase.from('listings').select(`*, states (name), districts (name)`).eq('listing_type', 'education').eq('status', 'approved').eq('verification_status', 'verified').order('created_at', { ascending: false })
  ...
  return <main className="min-h-screen bg-gray-50"><div className="max-w-6xl mx-auto px-4 py-8 md:py-10">
    ...
    <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-8"><p className="font-semibold text-gray-900">Education Information Notice</p><p className="text-sm text-gray-700 mt-2 leading-6">Vallalar Jeevakarunyam provides listing information only. Please contact the respective education provider directly to confirm courses, eligibility, admission, fees, scholarships, timings and availability. Priority is given to free, nominal-fee and genuinely affordable educational services.</p></div>
