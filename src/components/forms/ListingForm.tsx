...
export default function ListingForm() {
  ...
  const handlePhotoChange=(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    if(selected.length>MAX_FILES){setErrorMessage(`You can upload up to ${MAX_FILES} photos or PDF documents.`);return}
    const invalid=selected.find(file=>!['image/jpeg','image/png','image/webp','application/pdf'].includes(file.type)||file.size>MAX_FILE_SIZE)
    if(invalid){setErrorMessage('Each file must be JPEG, PNG, WebP or PDF and no larger than 5 MB.');return}
    setPhotos(selected)
  }
  ...
  const handleSubmit=async(e:React.FormEvent<HTMLFormElement>)=>{
    ...
    try{
      ...
      const documents=result?.uploadedDocumentCount||0
      const failed=(result?.photoErrors?.length||0)+(result?.documentErrors?.length||0)
      setSuccessMessage(failed?`Listing submitted successfully with ${uploaded} photo${uploaded===1?'':'s'} and ${documents} PDF document${documents===1?'':'s'}. ${failed} file${failed===1?'':'s'} could not be uploaded; an admin can add them later.`:`Listing submitted successfully with ${uploaded} photo${uploaded===1?'':'s'} and ${documents} PDF document${documents===1?'':'s'}. It is now pending admin review.`)
  ...
  return <form onSubmit={handleSubmit} onReset={handleReset} className="space-y-6">
    ...
    <div className="border border-emerald-200 bg-emerald-50/50 rounded-xl p-5 md:p-6"><h3 className="text-lg font-semibold text-gray-900">Listing Photos & Documents</h3><p className="text-sm text-gray-600 mt-1 mb-4">Add up to 5 clear photos or PDF documents about the place, organisation or service. Files are stored securely and will be shown publicly only after the listing is approved.</p><input type="file" accept={ACCEPTED_FILE_TYPES} multiple onChange={handlePhotoChange} disabled={loading} className="block w-full text-sm text-gray-700 file:mr-4 file:rounded-lg file:border-0 file:bg-emerald-700 file:px-4 file:py-2 file:font-semibold file:text-white hover:file:bg-emerald-800 disabled:opacity-50"/><p className="mt-2 text-xs text-gray-500">JPEG, PNG or WebP photos • PDF documents • Maximum 5 MB each • Up to 5 files</p>{photos.length>0&&<div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">{photos.map((photo,index)=><div key={`${photo.name}-${index}`} className="relative rounded-lg border border-gray-200 bg-white p-2">{photo.type==='application/pdf'?<div className="h-28 w-full rounded bg-gray-100 flex flex-col items-center justify-center text-red-700"><span className="text-3xl">📄</span><span className="text-xs font-semibold mt-1">PDF</span></div>:<img src={URL.createObjectURL(photo)} alt={photo.name} className="h-28 w-full object-cover rounded"/>}<button type="button" onClick={()=>removePhoto(index)} disabled={loading} className="absolute right-1 top-1 rounded-full bg-red-600 px-2 py-1 text-xs font-bold text-white shadow">Remove</button><p className="mt-1 truncate text-xs text-gray-600">{photo.name}</p></div>)}</div>}</div>
