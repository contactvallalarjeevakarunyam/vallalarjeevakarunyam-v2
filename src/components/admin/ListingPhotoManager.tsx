...
export default function ListingPhotoManager({ listingId, initialCoverImageUrl }: { listingId: number; initialCoverImageUrl?: string | null }) {
  ...
  async function loadImages() {
    ...
    if (response.ok) {
      ...
      setLoaded(true)
    } else setMessage('Unable to load listing photos.')
    setLoading(false)
  }
  ...
  return <section className="mt-4 rounded-xl border border-violet-200 bg-violet-50/50 p-4">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><h4 className="font-semibold text-gray-900">📷 Listing Photos</h4><p className="text-xs text-gray-600 mt-1">Available before or after approval. Newest upload becomes the public cover.</p></div>
      <button type="button" onClick={() => void toggle()} className="px-3 py-2 border border-violet-300 bg-white text-violet-800 text-sm font-semibold rounded-lg hover:bg-violet-50">{opened ? 'Close' : cover ? 'Manage Photos' : 'Add Photos'}</button>
    </div>

    {opened && <div className="mt-4">
      {loading ? <p className="text-sm text-gray-600">Loading photos…</p> : images.length > 0 ? <div className="grid grid-cols-2 md:grid-cols-4 gap-3">{images.map(image => <div key={image.id} className="relative rounded-lg overflow-hidden border border-violet-200 bg-white">
      ...
      </div>)}</div> : !loading && <p className="text-sm text-gray-600">No photos added yet.</p>}
